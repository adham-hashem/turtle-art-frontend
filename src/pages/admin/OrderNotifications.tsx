  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center py-12">
        ...
        <span className="mr-3 text-[#8B7355] font-medium" style={{ fontFamily: 'Tajawal, sans-serif' }}>جاري تحميل الإشعارات...</span>
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-bold text-[#8B7355]" style={{ fontFamily: 'Tajawal, sans-serif' }}>إشعارات الطلبات</h2>
      ...
      {selectedNotification ? (
        <div className="mb-6 p-6 bg-gradient-to-br from-white to-[#FAF9F6] rounded-2xl shadow-xl border-2 border-[#E5DCC5]">
          ...
          <div className="space-y-4">
            <div className="p-4 bg-white rounded-xl border-2 border-[#E5DCC5]">
              <span className="text-sm text-[#8B7355]/70 block mb-1" style={{ fontFamily: 'Tajawal, sans-serif' }}>العنوان</span>
              <p className="font-semibold text-[#8B7355] text-lg" style={{ fontFamily: 'Tajawal, sans-serif' }}>{selectedNotification.title}</p>
            </div>

            <div className="p-4 bg-white rounded-xl border-2 border-[#E5DCC5]">
              <span className="text-sm text-[#8B7355]/70 block mb-1" style={{ fontFamily: 'Tajawal, sans-serif' }}>الرسالة</span>
              <p className="text-[#8B7355]" style={{ fontFamily: 'Tajawal, sans-serif' }}>{formatNotificationBody(selectedNotification.body)}</p>
            </div>

            <div className="flex items-center gap-3 p-4 bg-white rounded-xl border-2 border-[#E5DCC5]">
              <Calendar className="h-5 w-5 text-[#D4AF37]" />
              <div className="flex-1">
                <span className="text-sm text-[#8B7355]/70 block" style={{ fontFamily: 'Tajawal, sans-serif' }}>تاريخ الإنشاء</span>
                <p className="font-semibold text-[#8B7355]" style={{ fontFamily: 'Tajawal, sans-serif' }}>{formatDate(selectedNotification.sentAt)}</p>
              </div>
            </div>

            <div className="p-4 bg-white rounded-xl border-2 border-[#E5DCC5]">
              <span className="text-sm text-[#8B7355]/70 block mb-2" style={{ fontFamily: 'Tajawal, sans-serif' }}>الحالة</span>
            ...
            {selectedNotification.errorMessage && (
              <div className="flex items-start gap-3 p-4 bg-red-50 rounded-xl border-2 border-red-200">
                ...
                <div className="flex-1">
                  <span className="text-sm text-red-600 font-semibold block mb-1" style={{ fontFamily: 'Tajawal, sans-serif' }}>رسالة الخطأ</span>
                  <p className="text-red-700" style={{ fontFamily: 'Tajawal, sans-serif' }}>{selectedNotification.errorMessage}</p>
                </div>
              </div>
            )}
          </div>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full">
                <thead className="bg-gradient-to-r from-[#F5F5DC] to-[#E5DCC5]">
                  <tr>
                    <th className="px-6 py-4 text-right text-xs font-bold text-[#8B7355] uppercase tracking-wider" style={{ fontFamily: 'Tajawal, sans-serif' }}>العنوان</th>
                    <th className="px-6 py-4 text-right text-xs font-bold text-[#8B7355] uppercase tracking-wider" style={{ fontFamily: 'Tajawal, sans-serif' }}>الرسالة</th>
                    <th className="px-6 py-4 text-right text-xs font-bold text-[#8B7355] uppercase tracking-wider" style={{ fontFamily: 'Tajawal, sans-serif' }}>التاريخ</th>
                    ...
                    <tr key={notification.id} className="hover:bg-[#FAF9F6] transition-colors">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`text-sm ${notification.isRead ? 'text-[#8B7355]/70' : 'font-bold text-[#8B7355]'}`} style={{ fontFamily: 'Tajawal, sans-serif' }}>
                            {notification.title}
                          </span>
                        </div>
                      </td>
                      <td className={`px-6 py-4 text-sm ${notification.isRead ? 'text-[#8B7355]/70' : 'font-semibold text-[#8B7355]'}`} style={{ fontFamily: 'Tajawal, sans-serif' }}>
              ...
              >
                <div className="flex justify-between items-start mb-3">
                  <div className="flex items-start gap-2 flex-1">
                    ...
                    <div>
                      <p className={`text-sm ${notification.isRead ? 'text-[#8B7355]/70' : 'font-bold text-[#8B7355]'}`} style={{ fontFamily: 'Tajawal, sans-serif' }}>
                        {notification.title}
                      </p>
                ...
                <div className="space-y-2 border-t-2 border-[#F5F5DC] pt-3">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-xs text-[#8B7355] font-medium" style={{ fontFamily: 'Tajawal, sans-serif' }}>{formatDate(notification.sentAt)}</span>
                  </div>
                  <div className="flex justify-between items-center">
        ...
        <div className="text-center py-16 bg-gradient-to-br from-white to-[#FAF9F6] rounded-2xl shadow-xl border-2 border-[#E5DCC5]">
          ...
          <p className="text-xl font-bold text-[#8B7355] mb-2" style={{ fontFamily: 'Tajawal, sans-serif' }}>لا توجد إشعارات</p>
          <p className="text-[#8B7355]/70" style={{ fontFamily: 'Tajawal, sans-serif' }}>لم يتم العثور على أي إشعارات حالياً</p>
        </div>
      )}
    </div>
  );
