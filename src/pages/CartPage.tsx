...
const CartPage: React.FC = () => {
  ...
  const handleUpdateQuantity = async (itemId: string, newQuantity: number) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) throw new Error('جلسة منتهية، يرجى تسجيل الدخول مرة أخرى');
        if (response.status === 400) throw new Error('كمية غير صالحة');
        if (response.status === 404) throw new Error('العنصر غير موجود في السلة');
        if (response.status === 409) throw new Error('تم تعديل العنصر من قبل مستخدم آخر، يرجى إعادة المحاولة');
  ...
  const handleRemoveItem = async (itemId: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) throw new Error('جلسة منتهية، يرجى تسجيل الدخول مرة أخرى');
        if (response.status === 404) throw new Error('العنصر غير موجود في السلة');
        if (errorText.includes('REFERENCE constraint') || errorText.includes('Orders')) {
  ...
  const handleApplyDiscountCode = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text();
        if (response.status === 401) throw new Error('جلسة منتهية، يرجى تسجيل الدخول مرة أخرى');
        if (response.status === 400 || errorText.includes('invalid')) throw new Error('كود الخصم غير صحيح أو منتهي الصلاحية');
      ...
      if (discount <= 0) alert('كود الخصم غير صحيح أو منتهي الصلاحية');
      else alert(`تم تطبيق خصم ${discount}% بنجاح`);
    } catch (err) {
  ...
  // ✅ Wrap every UI state with BottomNav so it always appears
  ...
  if (loading) {
    return (
      <PageShell>
        <div
          className="min-h-screen bg-gradient-to-b from-[#FAF9F6] to-[#F5F5DC] flex items-center justify-center px-4 pt-20 pb-24"
          dir="rtl"
        >
  ...
  return (
    <PageShell>
      {/* ✅ pb-24 so content doesn't hide behind BottomNav */}
      <div className="min-h-screen bg-gradient-to-b from-[#FAF9F6] to-[#F5F5DC] py-4 sm:py-6 md:py-8 pt-24 pb-24" dir="rtl">
          ...
          {error && (
            <div className="bg-red-50 border-2 border-red-200 text-red-700 p-3 sm:p-4 rounded-xl sm:rounded-2xl mb-4 sm:mb-6 shadow-md">
              <div className="flex items-center justify-between">
                <span className="text-sm sm:text-base" style={{ fontFamily: 'Tajawal, sans-serif' }}>
                  {error}
                </span>
          ...
          <div className="flex flex-col lg:grid lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
            ...
            <div className="lg:col-span-2 bg-white rounded-2xl sm:rounded-3xl shadow-xl p-4 sm:p-6 border-2 border-[#E5DCC5]">
              <div className="flex justify-between items-center mb-4 sm:mb-6 pb-3 sm:pb-4 border-b-2 border-[#E5DCC5]">
                <h2
                  className="text-lg sm:text-xl md:text-2xl font-bold text-[#8B7355] flex items-center gap-2"
                  style={{ fontFamily: 'Tajawal, sans-serif' }}
                >
                ...
                <button
                  ...
                  style={{ fontFamily: 'Tajawal, sans-serif' }}
                >
                  {isClearingCart ? <Loader2 className="animate-spin ml-1 sm:ml-2" size={16} /> : <Trash2 className="ml-1 sm:ml-2" size={16} />}
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-2xl sm:rounded-3xl shadow-xl p-4 sm:p-6 sticky top-4 sm:top-24 border-2 border-[#E5DCC5]">
                ...
                <button
                  ...
                  className="w-full bg-gradient-to-r from-[#8B7355] to-[#A67C52] text-white py-3 sm:py-3.5 rounded-xl sm:rounded-2xl hover:from-[#6B5644] hover:to-[#8B6644] font-bold text-base sm:text-lg shadow-lg hover:shadow-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                  style={{ fontFamily: 'Tajawal, sans-serif' }}
                  disabled={isClearingCart}
                >
                  <ShoppingBag size={20} />
